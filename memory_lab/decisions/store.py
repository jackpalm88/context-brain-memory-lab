...
class DecisionStore:
    ...
    def list_conflicts(self) -> DecisionConflictsResponse:
        ...
        conflicts = [
            ConflictPair(
                decision_a=LineageNode(
                    decision_id=str(r["a_id"]),
                    title=r["a_title"],
                    decision_status=r["a_status"],
                    created_at=r["a_created"],
                ),
                decision_b=LineageNode(
                    decision_id=str(r["b_id"]),
                    title=r["b_title"],
                    decision_status=r["b_status"],
    ...
    def get_timeline(self, hub_id: Optional[UUID], tags: Optional[str], limit: int) -> DecisionTimelineResponse:
            ...
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute(
                    ...
                    SELECT decision_id, title, decision_status, reversible,
                     ...
                     WHERE (%s::uuid IS NULL OR %s::uuid = ANY(linked_hub_ids))
                       AND (%s::text[] IS NULL OR decision_tags && %s::text[])
                     ORDER BY created_at DESC
                     LIMIT %s
                    """,
                    (str(hub_id) if hub_id else None, str(hub_id) if hub_id else None, tag_list or None, tag_list or None, limit),
                )
                rows = cur.fetchall()
        ...
        return DecisionTimelineResponse(
            ...
            reversed=grouped["reversed"],
            draft=grouped["draft"],
            total=len(rows),
        )

    def list_decisions(self, status: Optional[str], hub_id: Optional[UUID], limit: int) -> DecisionListResponse:
        conditions: List[str] = []
        params: List[Any] = []
        if status:
        ...
        with self._conn() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                if payload.supersedes_decision_id:
                    cur.execute(
                        "SELECT 1 FROM cb_decision_nodes WHERE decision_id = %s::uuid",
                        (str(payload.supersedes_decision_id),),
                    )
                    if not cur.fetchone():
                ...
                cur.execute(
                    ...
                    INSERT INTO cb_decision_nodes (
                        title, decision_reason, decision_context, why_this_matters,
                ...
                if payload.supersedes_decision_id:
                    cur.execute(
                        ...
                        """,
                        (new_id, str(payload.supersedes_decision_id)),
                    )
            conn.commit()
        return row

    def get_lineage(self, decision_id: UUID) -> DecisionLineageResponse:
        with self._conn() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute(
                    "SELECT decision_id, title FROM cb_decision_nodes WHERE decision_id = %s::uuid",
                    (str(decision_id),),
                )
                base = cur.fetchone()
                if not base:
                ...
                cur.execute(
                    ...
                    WITH RECURSIVE ancestors AS (
                        SELECT decision_id, title, decision_status, created_at,
                               supersedes_decision_id, 1 AS depth
                          FROM cb_decision_nodes
                         WHERE decision_id = (
                             SELECT supersedes_decision_id FROM cb_decision_nodes WHERE decision_id = %s::uuid
                         )
                        UNION ALL
                        SELECT d.decision_id, d.title, d.decision_status, d.created_at,
                               d.supersedes_decision_id, a.depth + 1
                          FROM cb_decision_nodes d
                          JOIN ancestors a ON d.decision_id = a.supersedes_decision_id
                         WHERE a.depth < 10
                           AND a.supersedes_decision_id IS NOT NULL
                    ),
                    descendants AS (
                        SELECT decision_id, title, decision_status, created_at,
                               superseded_by_decision_id, 1 AS depth
                          FROM cb_decision_nodes
                         WHERE decision_id = (
                             SELECT superseded_by_decision_id FROM cb_decision_nodes WHERE decision_id = %s::uuid
                         )
                        UNION ALL
                        SELECT d.decision_id, d.title, d.decision_status, d.created_at,
                               d.superseded_by_decision_id, desc2.depth + 1
                          FROM cb_decision_nodes d
                          JOIN descendants desc2 ON d.decision_id = desc2.superseded_by_decision_id
                         WHERE desc2.depth < 10
                           AND desc2.superseded_by_decision_id IS NOT NULL
                    )
        ...
        ancestors = [
            ...
            for r in rows if r["role"] == "ancestor"
        ]
        descendants = [
            LineageNode(
                decision_id=str(r["decision_id"]),
            ...
            for r in rows if r["role"] == "descendant"
        ]
        ...
        return DecisionLineageResponse(
            ...
            depth=max_depth,
            depth_limit_reached=max_depth >= 10,
        )

    def explain_decision(self, decision_id: UUID) -> DecisionFull:
        with self._conn() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute("SELECT * FROM cb_decision_nodes WHERE decision_id = %s::uuid", (str(decision_id),))
                row = cur.fetchone()
        if not row:
            raise KeyError(f"Decision {decision_id} not found")
        return self._row_to_full(row)

    def update_status(self, decision_id: UUID, decision_status: str) -> DecisionFull:
        with self._conn() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute(
                    """
                 ...
                 RETURNING *
                    """,
                    (decision_status, str(decision_id)),
                )
                row = cur.fetchone()
            conn.commit()
        if not row:
