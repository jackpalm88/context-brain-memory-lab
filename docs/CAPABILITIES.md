...
- **MCP surface** — 34 MCP tools are registered in the public Memory Lab MCP surface: 32 approved Context Brain-parity names plus 2 public-only additions (`list_current_state_anchors`, CF-003; `list_decisions_for_content`, CF-002). *(Corrected 2026-07-22: prior text said 32/32 — stale since the 2026-07-08 CF additions.)*
...
- **MCP ergonomics** — all 34 MCP tools ship useful descriptions; `query_memory` can opt into the provider-backed ask mode and self-explains when the deployment gate keeps it deterministic.
...
- **First-class scoped retrieval** (see [DESIGN_SCOPED_RETRIEVAL.md](DESIGN_SCOPED_RETRIEVAL.md)) — an optional `retrieval_scope` envelope (`allowed_hubs`, `content_types`) is accepted by `RetrievalRequest`, `AskRequest`, `memory_lab_retrieval_search`, and `query_memory`. It restricts the candidate set BEFORE scoring across all three retrieval paths (pgvector, deterministic/lexical, hub-linked), reusing the existing `cb_hub_content` junction and its indexes — no new schema, no ranking change. Enforcement is fail-closed by construction: a nonexistent hub, a zero-content hub, or a workspace-mismatched hub yields zero candidates from that source, never a silent unscoped fallback. `content_types` is an alias for the pre-existing `memory_type`/`memory_types` filter expressed inside the scope envelope; equivalent legacy and scoped values may coexist, contradictory ones fail validation (422). The response echoes what was actually enforced under `scope_applied` (`{allowed_hubs, content_types, enforcement: "pre_filter"}`), always present when a scope was supplied; `/v1/ask` audit rows additionally record `requested_scope` and `scope_enforcement`. Omitting `retrieval_scope` is byte-identical to prior behavior. `subject_scope`/`policy_scope` are reserved names only — no backing schema exists for either.
